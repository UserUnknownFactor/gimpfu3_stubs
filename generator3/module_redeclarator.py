import os, sys, traceback, enum, gc, traceback
sys.path.extend([os.path.join(os.path.dirname(os.path.realpath(__file__)), ".")])
from util_methods import *
from docstring_parsing import *
...
class ModuleRedeclarator(object):
    def __init__(self, module, mod_qname, mod_filename, cache_dir, indent_size=4, doing_builtins=False):
        ...
        self.test_mode = False#generator3.core.is_test_mode()
        self.gen_version = os.environ.get(ENV_VERSION)#generator3.core.version()
        self.module = module
    ...
    def get_ret_value_for_type(self, attrs):
        ...
        for attr in attrs.split(','):
            ...
            if attr:
                ...
                if ret_type:
                    ret_value.append(ret_type)
                    continue
                thing = getattr(self.module, attr, None)
                if thing:
                    if not isinstance(thing, type) and is_callable(thing): # a function
                        ret_value.append(None) # TODO: maybe divinate a return type; see pygame.mixer.Channel
                    ret_value.append(attr)
                    continue
    ...
    def restore_by_docstring(self, signature_string, class_name, deco=None, ret_hint=None):
        ...
        """
        action(f"restoring func-sig {signature_string} of class {class_name} with decorator {deco}")
        # parse
        ...
        try:
            ...
            if tokens:
                ...
                if ret_t[0] is T_RETURN:
                    _rets = ret_t[1:]
                    _lrets = len(_rets)
                    for i, _r in enumerate(_rets):
                        _r_tmp = []
                        for p in _r.split(','):
                             _r_tmp.append(p)#remove_prefix(p, ":"))
                        if len(_r_tmp) > 1:
                            _r_tmp = 'tuple[' + ', '.join(_r_tmp) + ']'
                        else:
                            if _lrets > 1:
                                _r_tmp = _r_tmp[0]
                            else:
                                _r_tmp = remove_prefix(_r_tmp[0], ':')
                        _rets[i] = _r_tmp
                    if len(_rets) > 1:
                        ret_spec = 'Union[' + ', '.join(_rets) + ']'
                    else:
                        ret_spec = _rets[0]
            ret_value = None #self.get_ret_value_for_type(_rets) or self.get_ret_value_for_type(_rets)
            ...
            try:
                ...
                pass
                #
        seq = transform_tokens_into_spec_string(tokens)

        # add safe defaults for unparsed
        if parsing_failed:
            doc_node = self.SIG_DOC_UNRELIABLY ## restored from __doc__ memo
            starred = None
            double_starred = None
            for one in seq:
                if type(one) is str:
        ...
        if class_name and (not seq or seq[0] != 'self'):
            first_param = None
            #if signature_string and '(self,' not in signature_string:
            #    first_param = propose_first_param(deco)
            if first_param:
                seq.insert(0, first_param)
        seq = make_names_unique(seq)
        return (seq, ret_value, ret_spec, doc_node)
    ...
    def parse_func_doc(self, func_doc, func_id, func_name, class_name, deco=None, sip_generated=False):
        ...
        @param class_name: name of the containing class, or None
        @param deco: decorator to use
        @return (reconstructed_spec, return_literal, ret_type note) or (None, _, _, _) if failed.
        """
        if sip_generated:
            overloads = []
            ...
            if len(overloads) > 1:
                ...
                for result in docstring_results:
                    ...
                    if rt and rt not in ret_types:
                        ret_types.append(rt)
                if ret_types:
                    ret_literal = "Union[" + ", ".join(ret_types) + "]"
                else:
                    ret_literal = None
                ...
                return (spec, ret_literal, \
                    "tuple[" + ", ".join(ret_types) + "]", \
                    "restored from __doc__ with multiple overloads")

        # find the first thing to look like a definition
        _func_id_filtered = re.sub(r'^do_', '', re.escape(func_id))
        _func_id_filtered = re.sub(r'^__(.+)__$', r'\1', re.escape(_func_id_filtered))
        prefix_re = re.compile(r"\s*(?:(\w+)[ \t]+)?(?:do_|__)?" + _func_id_filtered + r"(?:__)?\s*\(") # "foo(..." or "int foo(..."
        ...
        if match:
            ...
            params, ret_literal, ret_type, doc_note = self.restore_by_docstring(func_doc[match.end():], class_name, deco, ret_hint)
            spec = func_name + flatten(params)
            return (spec, ret_literal, ret_type, doc_note)
        else:
    ...
    def redo_function(self, out, p_func, p_name, indent, p_class=None, p_modname=None, classname=None, seen=None):
        ...
        if inspect and inspect.isfunction(p_func):
            ...
            if isinstance(funcdoc, STR_TYPES):
                (spec, ret_literal, ret_type, more_notes) = self.parse_func_doc(
                ...
                if more_notes:
                    ...
                    #if hasattr(p_func, "__gdoc__"):
                    #    funcdoc = p_func.__gdoc__
                    sig_note += more_notes
            ...
            if p_class and (not '(self,' in spec and not '(self)' in spec) and "staticmethod" != deco:
                deco = "staticmethod"
                out(indent, "@", deco, deco_comment)

            out(indent, f"def {spec} {('-> '+ ret_type) if ret_type else ''}: # {sig_note}")
            # to reduce size of stubs, don't output same docstring twice for class and its __init__ method
            if not is_init:
                if funcdoc != p_class.__doc__:
    ...
    def redo_class(self, out, p_class, p_name, indent, p_modname=None, seen=None, inspect_dir=False):
        ...
        field_source = {}
        try:
            if hasattr(p_class, "__enum_values__"):
                field_source = p_class.__enum_values__
                for i, k in field_source.items():
                    out(indent + 1, f"{k.value_name} = {i}")
                field_keys = ()
            elif hasattr(p_class, "__flags_values__"):
                field_source = p_class.__flags_values__
                for i, k in field_source.items():
                    _name = k.value_names[-1]
                    out(indent + 1, f"{_name} = {i}")
                field_keys = ()
        ...
        for item_name in field_keys:
            ...
            if item_qname in CLASS_ATTR_BLACKLIST or item_name in UNIVERSAL_ATTR_BLACKLIST:
                note('skipping blacklisted attribute ' + item_qname)
                continue
                item = field_source.get(item_name)
            if item_name in ("__gdoc__", "__doc__", "__module__"):
    ...
    def redo_simple_header(self, p_name):
        ...
        #import generator3.core
        if ENV_PREGENERATION_MODE_FLAG in os.environ:
    ...
    def redo(self, p_name, inspect_dir):
        ...
        for item_name in module_dict:
            note(f"looking at {item_name}")
            if item_name in (
                "__dict__", "__doc__", "__gdoc__", "__gtype__", "__enum_values__", 
                "__flags_values__", "__info__", "__module__", "__file__", "__name__", 
            ...
            if not want_to_import:
                ...
                is_class = type(item).__name__ == 'classobj'
                if is_type or is_class:
