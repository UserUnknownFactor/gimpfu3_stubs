...
from util_methods import sanitize_value
...
simpleParamSeq = ZeroOrMore(paramSlot + NOCOMMA) + Optional(paramSlot + Optional(NOCOMMA))
nestedParamSeq = Group(
    Suppress('(').setParseAction(replaceWith(T_NESTED)) + \
    simpleParamSeq + Optional(ellipsis + Optional(NOCOMMA) + Optional(simpleParamSeq)) + \
    Suppress(')')
)  # we accept "(a1, ... an)"
...
optionalPart << (
        Group(
            Suppress('[').setParseAction(replaceWith(T_OPTIONAL)) + Optional(NOCOMMA) + \
...
return_type = Group(
    ...
    Suppress(NOSP + (Literal("->") | (Literal(":") + NOSP + Literal("return"))) + NOSP) + \
        spaced_comma_ident + ZeroOrMore(NOSP + Suppress(Literal("or")) + NOSP + spaced_comma_ident)
)
...
def transform_tokens_into_spec_string(results, toplevel=True):
    ...
    for i, token in enumerate(results):
        ...
        if token_type is T_SIMPLE:
            ...
            if len(token) == 3: # name with value
                if toplevel:
                    _ret = sanitize_ident(token_name, is_clr)
                    if all(len(j) == 3 for j in results[i+1:]) or i == lres:
                        _ret += "=" + sanitize_value(token[2])
                    ret.append(_ret)
                else:
            ...
            inner = transform_tokens_into_spec_string(token[1:], False)
            if len(inner) != 1:
                ret.append(inner)
            else:
...
def sanitize_ident(x, is_clr=False, needs_sanitize=False):
    """Takes an identifier and returns it sanitized"""
    if not needs_sanitize: return x
