...
def out_doc_attr(out_func, p_object, indent, p_class=None):
    ...
    if p_class and not the_doc:
        the_doc = getattr(p_class, "__gdoc__", None)
    if the_doc:
...
def remove_prefix(parameter, initchar):
    if not initchar or not parameter or initchar not in parameter:
        return parameter
    return parameter[parameter.index(initchar)+1:]

def remove_suffix(parameter, initchar):
    if not initchar or not parameter or initchar not in parameter:
        return parameter
    return parameter[:parameter.index(initchar)]
...
##############  notes, actions #################################################################
_is_verbose = False#sys.platform == 'cli' # controlled by -v
